...
There are five class-specific commands for the preamble\sidenote{The \texttt{sidenote} command has been provided for adding notes to the margin}. Table~(\ref{tab:the-only-table}) describes these commands\footnote{Alternatively, \texttt{footnote} can be used}. Some convenience macros such as \texttt{C}, \texttt{L}, \texttt{R}, \texttt{D[<length>]} have been provided for aligning the text within the columns of a table.
\begin{table}[h]
  \centering
